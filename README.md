...
- [x] AI生成流程优化（移除自动保存）
- [x] 设置持久化（自动保存和加载用户设置）
- [x] 预览图尺寸保持（下载高分辨率图片时预览图尺寸不变）
- [x] 生成历史记录功能：保存所有生成的文件到沙盒，支持查看、筛选、删除和分享
...
- [ ] 云端同步
- [ ] 更多AI模型
- [ ] **从相册选择图片进行二次开发**：支持用户从相册选择一张图片，然后在此基础上进行图标设计和编辑

## 📈 最新更新

### 2024-12-25 更新
- **生成历史记录功能**：新增历史记录管理，所有生成的文件自动保存到沙盒，使用时间戳命名（yyyyMMddHHmmssSSS）
- **历史查看界面**：支持全部/图片/ZIP三种类型筛选，展示文件名、时间、大小等信息
- **批量管理**：支持批量选择和删除历史文件，支持单个文件分享
- **优化设置页面布局**：统一设置块间距，调整文字与滑块对齐

### 2024-10-12 更新
- **自定义文字大小输入框优化**：修复了自定义字体大小输入框总是显示100的问题，现在用户可以正常编辑自定义字体大小
- **AI生成流程优化**：移除了AI生成图标时的自动保存功能，现在点击"生成图标"按钮只是生成图标并关闭弹窗，不会自动保存到相册
- **用户体验提升**：优化了文字设置弹窗的交互逻辑，避免意外的保存操作
